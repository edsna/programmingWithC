/*
 * Edson Zandamela CPSC 275
 * This program interprets instructions to test the utilities of a machine (the SM275)
 */
#include<stdio.h>
#include<string.h>
#include<stdlib.h>
...
void main(){  
    ...
    int data;								//Data
    char MemLoc[MaxElemInp];				//Memory Location
	...
	int i = 0;
	//Beggining of interactivity
	while(strcasecmp(action, "QUIT") != 0){
	                printf(">> ");
			scanf("%s", action); //store input in data
	        if(strcasecmp(action, "READ") == 0){				
			scanf("%x %s", &data, MemLoc); //store input in data
			//Check input correctness
				...
				scanf(" %s", MemLoc); //store input in data
					...
					if(strcasecmp(MemLoc, MemoryLocArr[i]) == 0){
						printf("Value stored at %s is: 0x%x \n", MemoryLocArr[i], StorArr[i]);
					}
				}
				...
				char first[10];	//defines dest of MOVE
				int j = 0;
				int k = 0;
				int z = 0;
				scanf(" %s %s", first, SecLoc); //store input in data
				printf("First Arg of first has: %c \n", first[0]);
				...
				char SecondAdd[MaxElemInp];	//defines dest of MOVE
				int a = 0;
				int d = 0;
				...
				int m = 0;
				int l = 0;
				scanf(" %s %s", firstMult, SecondMult); //store input in data
				//Check Correctness
